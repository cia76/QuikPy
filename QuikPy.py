...
class QuikPy(metaclass=Singleton):  # Если требуется одновременный доступ из разных скриптов к одному терминалу QUIK через QuikPy, то используйте Singleton класс
    ...
    def __init__(self, host='127.0.0.1', requests_port=34130, callbacks_port=34131):
        ...
        self.callback_thread = Thread(target=self.callback_handler,
                                      name='CallbackThread').start()  # Создаем и запускаем поток обработки функций обратного вызова
        ...
        for account in self.get_trade_accounts()['data']:  # Пробегаемся по всем торговым счетам
            ...
            client_code = next(
                ...
                None)  # Код клиента
            class_codes: list[str] = account['class_codes'][1:-1].split(
                '|')  # Список режимов торгов счета. Убираем первую и последнюю вертикальную черту, разбиваем по вертикальной черте
            self.accounts.append(dict(  # Добавляем торговый счет
                client_code=client_code, firm_id=firm_id, trade_account_id=account['trdaccid'],
                # Код клиента / Фирма / Счет
                class_codes=class_codes,
                futures=(firm_id == self.futures_firm_id)))  # Режимы торгов / Счет срочного рынка
    ...
    def is_connected(self,
                     trans_id=0):  # 2.1.1 Функция предназначена для определения состояния подключения клиентского места к серверу
    ...
    def get_script_path(self,
                        trans_id=0):  # 2.1.2 Функция возвращает путь, по которому находится запускаемый скрипт, без завершающего обратного слеша (\). Например, C:\QuikFront\Scripts
        """Путь скрипта
    ...
    def get_info_param(self, params,
                       trans_id=0):  # 2.1.3 Функция возвращает значения параметров информационного окна (пункт меню Система / О программе / Информационное окно…)
    ...
    def get_working_folder(self,
                           trans_id=0):  # 2.1.6 Функция возвращает путь, по которому находится файл info.exe, исполняющий данный скрипт, без завершающего обратного слеша (\). Например, c:\QuikFront
    ...
    def get_classes_list(self,
                         trans_id=0):  # 3.2.1 Функция предназначена для получения списка режимов торгов, переданных с сервера в ходе сеанса связи
        """Все режимы торгов
    ...
    def get_class_info(self, class_code,
                       trans_id=0):  # 3.2.2 Функция предназначена для получения информации о режиме торгов
    ...
    def get_class_securities(self, class_code,
                             trans_id=0):  # 3.2.3 Функция предназначена для получения списка кодов инструментов для списка режимов торгов, заданного списком кодов
    ...
    def get_money(self, client_code, firm_id, tag, curr_code,
                  trans_id=0):  # 3.3.1 Функция предназначена для получения информации по денежным позициям
        """Денежные позиции
    ...
    def get_money_ex(self, firm_id, client_code, tag, curr_code, limit_kind,
                     trans_id=0):  # 3.3.2 Функция предназначена для получения информации по денежным позициям указанного типа
    ...
    def get_depo(self, client_code, firm_id, sec_code, account,
                 trans_id=0):  # 3.4.1 Функция предназначена для получения позиций по инструментам
    ...
    def get_depo_ex(self, firm_id, client_code, sec_code, account, limit_kind,
                    trans_id=0):  # 3.4.2 Функция предназначена для получения позиций по инструментам указанного типа
    ...
    def get_futures_limit(self, firm_id, account_id, limit_type, curr_code,
                          trans_id=0):  # 3.5.1 Функция предназначена для получения информации по фьючерсным лимитам
        """Фьючерсные лимиты
    ...
    def get_futures_holding(self, firm_id, account_id, sec_code, position_type,
                            trans_id=0):  # 3.6.1 Функция предназначена для получения информации по фьючерсным позициям
        """Фьючерсные позиции
    ...
    def get_security_info(self, class_code, sec_code,
                          trans_id=0):  # 3.7.1 Функция предназначена для получения информации по инструменту
    ...
    def get_quote_level2(self, class_code, sec_code,
                         trans_id=0):  # 3.9.1 Функция предназначена для получения стакана по указанному режиму торгов и инструменту
    ...
    def get_num_candles(self, tag,
                        trans_id=0):  # 3.10.2 Функция предназначена для получения информации о количестве свечек по выбранному идентификатору
    ...
    def get_candles_from_data_source(self, class_code, sec_code, interval, param='-', count=0):
        """Свечи
        ...
        :param int count: Кол-во свечей. 0 - все
        """
        return self.process_request({'data': f'{class_code}|{sec_code}|{interval}|{param}|{count}', 'id': '1',
                                     'cmd': 'get_candles_from_data_source', 't': ''})
    ...
    def subscribe_to_candles(self, class_code, sec_code, interval, param='-', trans_id=0):  # QUIK#
        ...
        """
        return self.process_request(
            {'data': f'{class_code}|{sec_code}|{interval}|{param}', 'id': trans_id, 'cmd': 'subscribe_to_candles',
             't': ''})
    ...
    def is_subscribed(self, class_code, sec_code, interval, param='-', trans_id=0):  # QUIK#
        ...
        """
        return self.process_request(
            {'data': f'{class_code}|{sec_code}|{interval}|{param}', 'id': trans_id, 'cmd': 'is_subscribed', 't': ''})
    ...
    def unsubscribe_from_candles(self, class_code, sec_code, interval, param='-', trans_id=0):  # QUIK#
        ...
        """
        return self.process_request(
            {'data': f'{class_code}|{sec_code}|{interval}|{param}', 'id': trans_id, 'cmd': 'unsubscribe_from_candles',
             't': ''})

    # 3.11 Функции для работы с заявками

    def send_transaction(self, transaction,
                         trans_id=0):  # 3.11.1 Функция предназначена для отправки транзакций в торговую систему
    ...
    def get_param_ex(self, class_code, sec_code, param_name,
                     trans_id=0):  # 3.12.1 Функция предназначена для получения значений всех параметров биржевой информации из таблицы Текущие торги. С помощью этой функции можно получить любое из значений Таблицы текущих торгов для заданных кодов класса и инструмента
    ...
    def get_param_ex2(self, class_code, sec_code, param_name,
                      trans_id=0):  # 3.12.2 Функция предназначена для получения значений всех параметров биржевой информации из Таблицы текущих торгов с возможностью в дальнейшем отказаться от получения определенных параметров, заказанных с помощью функции ParamRequest
    ...
    def get_portfolio_info(self, firm_id, client_code,
                           trans_id=0):  # 3.13.1 Функция предназначена для получения значений параметров таблицы Клиентский портфель, соответствующих идентификатору участника торгов firmid, коду клиента client_code и сроку расчетов limit_kind со значением 0
        """Клиентский портфель
    ...
    def get_portfolio_info_ex(self, firm_id, client_code, limit_kind,
                              trans_id=0):  # 3.13.2 Функция предназначена для получения значений параметров таблицы Клиентский портфель, соответствующих идентификатору участника торгов firmid, коду клиента client_code и сроку расчетов limit_kind со значением, заданным пользователем.
    ...
    def add_label(self, price, cur_date, cur_time, qty, path, chart_tag, alignment, background,
                  trans_id=0):  # 3.16.1 Добавляет метку с заданными параметрами
        """Добавление метки на график"""
        return self.process_request(
            {'data': f'{price}|{cur_date}|{cur_time}|{qty}|{path}|{chart_tag}|{alignment}|{background}', 'id': trans_id,
             'cmd': 'AddLabel', 't': ''})
    ...
    def del_all_labels(self, chart_tag,
                       trans_id=0):  # 3.16.3 Команда удаляет все метки на диаграмме с указанным графиком
    ...
    def subscribe_level2_quotes(self, class_code, sec_code,
                                trans_id=0):  # 3.17.1 Функция заказывает на сервер получение стакана по указанному классу и инструменту
        """Подписка на стакан
    ...
    def unsubscribe_level2_quotes(self, class_code, sec_code,
                                  trans_id=0):  # 3.17.2 Функция отменяет заказ на получение с сервера стакана по указанному классу и инструменту
    ...
    def is_subscribed_level2_quotes(self, class_code, sec_code,
                                    trans_id=0):  # 3.17.3 Функция позволяет узнать, заказан ли с сервера стакан по указанному классу и инструменту
    ...
    def param_request(self, class_code, sec_code, param_name,
                      trans_id=0):  # 3.18.1 Функция заказывает получение параметров Таблицы текущих торгов
    ...
    def cancel_param_request(self, class_code, sec_code, param_name,
                             trans_id=0):  # 3.18.2 Функция отменяет заказ на получение параметров Таблицы текущих торгов
    ...
    def param_request_bulk(self, class_sec_codes_params, trans_id=0):  # QUIK#
        ...
        """
        return self.process_request(
            {'data': class_sec_codes_params, 'id': trans_id, 'cmd': 'paramRequestBulk', 't': ''})
    ...
    def cancel_param_request_bulk(self, class_sec_codes_params, trans_id=0):  # QUIK#
        ...
        """
        return self.process_request(
            {'data': class_sec_codes_params, 'id': trans_id, 'cmd': 'cancelParamRequestBulk', 't': ''})

    # 3.19 Функции для получения информации по единой денежной позиции

    def get_trd_acc_by_client_code(self, firm_id, client_code,
                                   trans_id=0):  # 3.19.1 Функция возвращает торговый счет срочного рынка, соответствующий коду клиента фондового рынка с единой денежной позицией
    ...
    def get_client_code_by_trd_acc(self, firm_id, trade_account_id,
                                   trans_id=0):  # 3.19.2 Функция возвращает код клиента фондового рынка с единой денежной позицией, соответствующий торговому счету срочного рынка
    ...
    def is_ucp_client(self, firm_id, client,
                      trans_id=0):  # 3.19.3 Функция предназначена для получения признака, указывающего имеет ли клиент единую денежную позицию
    ...
    def process_request(self, request):
        ...
        """
        raw_data = f'{request}\r\n'.replace("'", '"').encode(
            'cp1251')  # Переводим: словарь -> строка, одинарные кавычки -> двойные, кодировка UTF8 -> Windows 1251
    ...
    def dataname_to_class_sec_codes(self, dataname) -> Union[tuple[str, str], None]:
        ...
        if len(symbol_parts) >= 2:  # Если тикер задан в формате <Код режима торгов>.<Код тикера>
            ...
            class_code = self.get_security_class(class_codes, sec_code)[
                'data']  # Код режима торгов из всех режимов по тикеру
    ...
    def price_to_quik_price(self, class_code, sec_code, price) -> float:
        ...
        if class_code in ('TQOB', 'TQCB', 'TQRD',
                          'TQIR'):  # Для облигаций (Т+ Гособлигации, Т+ Облигации, Т+ Облигации Д, Т+ Облигации ПИР)
            quik_price = price * 100 / si[
                'face_value']  # Пункты цены для котировок облигаций представляют собой проценты номинала облигации
