...
def get_candles_from_provider(qp_provider, class_code, security_code, tf) -> pd.DataFrame:
    ...
    si = qp_provider.get_security_info(class_code, security_code)['data']  # Получаем информацию о тикере
    pd_bars['volume'] *= int(si['lot_size'])  # Объем в штуках
...
# noinspection PyShadowingNames
def save_candles_to_file(qp_provider, class_code, security_codes, tf,
